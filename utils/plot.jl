...
function plotVelocityField3d!( threeDPlot::Union{Scene,LScene}, fieldGenerator, objects::Vector{<:Object}, lat_den::Int64, lon_den::Int64 )::Nothing
    ...
    surface!(threeDPlot, map(s->s.x,sphereGrid), map(s->s.y,sphereGrid), map(s->s.z,sphereGrid), color=colorsGrid, colormap=:dense )
    nothing
end
...
function plotVelocityField2d!( twoDPlot::Union{Scene,LScene}, fieldGenerator, objects::Vector{<:Object}, lat_den::Int64, lon_den::Int64 )::Nothing
    ...
    heatmap!(twoDPlot, φs, θs, colorsGridTwoD, colormap=:dense)
    nothing
end
...
function animate(
    ...
    # objectState is dependent on slider so we 'lift' it to an observable
