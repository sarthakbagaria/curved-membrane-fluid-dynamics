...
dl is the length of dipole used in some tests but finally we take dl -> 0, so this should not appear in final functions
=#
...
v_t =   \gradient_{θ,ϕ} ( (force_t + force_t ⋅ [dθ_0, sin(θ_0)dϕ_0] ⋅ covd )  ⋅ \gradient_{θ_0+dθ_0, ϕ_0+dϕ_0} P(γ(θ,ϕ,θ_0+dθ_0,ϕ_0+dϕ_0)) )
       ...
       = \gradient_{θ,ϕ} (
              force_t ⋅ \gradient_{θ_0,ϕ_0} P(γ)  // 00 -> cancels with last term
            + force_t ⋅ ( ( [ dθ_0, dϕ_0] * [∂θ_0, ∂ϕ_0]' ) ( \gradient_{θ_0, ϕ_0} P(γ) ) ) /// 01
            + force_t ⋅ [dθ_0, sin(θ_0)dϕ_0] ⋅ covd ⋅ ( \gradient_{θ_0, ϕ_0} P(γ) )  // 10
            + force_t ⋅ ( ( [dθ_0, sin(θ_0)dϕ_0] ⋅ covd ) ⋅ ( [ dθ_0, dϕ_0] * [∂θ_0, ∂ϕ_0]' ) ( \gradient_{θ_0, ϕ_0} P(γ) ) ) /// 11 -> second order -> 0
...
function stepForward( object::DipoleForce, v::SphericalCoord, vc::Float64, dt::Float64 )::DipoleForce
    ...
    rotation due to curvature = cos(angle of v) * cot(θ) * distance moved by dipole
...
parallel transporting the force and velocity vectors along the geodesic connecting them,
...
# save compiled function to file to avoid recomputing this
